...
# Enabling s3 backend for terraform states (The bucket name below must exist)

terraform {
  backend "s3" {
    bucket         = "paas-clusters-terraform-states"
    key            = "production/production-terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
  }
}
...
locals {
  ...
  cluster_data = {
    ...
    cluster_sg = data.aws_security_group.sg1.id
