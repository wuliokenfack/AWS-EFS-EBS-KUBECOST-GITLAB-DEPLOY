# paas-jadeuc-clusters-terraform-scripts

This repo contains the Terraform code used to deploy the following clusters:
- Paas-local
- Paas-itss
- Paas-tooling
- Paas-production

Each cluster is built using one of the above modules and all states files are located in an S3 bucket. This Terraform code is not only configured to deploy one cluster at the time, but also to deploy all four clusters at the same time from the root module using "Terragrunt".
